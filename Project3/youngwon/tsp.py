...
class Chromosome:
    def __init__(self, g=[]):
        ...
        if self.genes.__len__()==0:	# 염색체가 초기 상태이면 초기화한다. 
            ...
            self.genes.append(new_list)        # 0 뒤에 갖다붙히기
    ...
    def cal_fitness(self):		# 적합도를 계산한다. 
        ...
        for i in range(SIZE-1):
            ...
            shortest_distance = DISTANCE[city1][city2]  # 0과 6, 즉 서울과 울산 사이의 최단 거리를 알아내기위해 DISTANCE 2차원 배열에서 [0][6]에 해당하는 값을 추출한다.
            sum_shortest_distance += shortest_distance  
...
def crossover(pop):
    ...
    fathercopy = father.genes[:]   # father 잠시 복사
    ...
    while (i<index1):
        ...
        fathercopy.remove([i])  #child1에 추가한 값들은 father에서 삭제
        i+=1
    child1.append(mother_extract) #index1에서 index2 사이의 위치에는 mother로부터 추출한 값들 입력
    child1.append(fathercopy) #나머지 index2 이후에는 father에 남아있는 값들을 입력
    ...
    while (i<index1):
        ...
        mothercopy.remove([i])
        i+=1
    child2.append(father_extract)
    child2.append(mothercopy)
...
def mutate(c):
    for i in range(SIZE):
        if random.random() < MUTATION_RATE:
            head = random.randint(1, SIZE) #서울 제외 랜덤하게 도시 두개 선택
            end = random.randint(1, SIZE)
            while (head == end):
                end = random.randint(1, SIZE)
            tmp = c.genes[head]
            c.genes[head] = c.genes[end]
            c.genes[end] = tmp    # 도시 두개 상호 교환
