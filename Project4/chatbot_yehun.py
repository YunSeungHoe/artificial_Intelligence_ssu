...
def keyword_check(df, com, rule, start_len, end_len): ### 동일한 이름이 들어간 음식을 구분하도록 ex)김치찌개, 참치 김치찌개
    matched_len = 0
    response = ""
    response_list = []
    for i in range(start_len, end_len):
        ...
        com = "".join(com)
        if rule == 1:
            search = re.search(compare_word, com)
            if search != None:
                span = search.span()
            ...
            if com in compare_word:
                random_list = random.sample(num_list, 5)
                for i in random_list:
                    response_list.append(df_recipe.values[i][WORD_COL])      
    if response:
...
keyword_flag, response = keyword_check(df, command_non_space, rule, START_ROW, END_ROW)
